...
//        NurseTFoodsOrder foodsorder = new NurseTFoodsOrder();
        ...
        try {
            ...
            String sql="";
            sql="Select foodsorder.*, department.department_namet "
        ...
        try {
            ...
            if(flag.equals("date")) {
                sql="Select foodsorder.*, department.department_namet, fop.foods_period_namet "
                    ...
                    +"Order By foodsorder."+ foodsorder.getFBranchId() +", foodsorder."+foodsorder.getFDateFoodsOrder()+", foodsorder."+foodsorder.getFWardNamet()
...
//                max = item.getBillLabId();
    ...
    public String CboFoodsOrderDetail(String branch_id,String foods_order_id, String selected){
        String txt="",sql="";
        Vector v_foods_order_detail = new Vector();
        NurseTFoodsOrderDetail foods_order_detail = new NurseTFoodsOrderDetail();
        v_foods_order_detail = getNurserFoodsOrderDetail(branch_id, foods_order_id, "");
        for(int i=0;i<=v_foods_order_detail.size()-1;i++) {
            foods_order_detail = (NurseTFoodsOrderDetail) v_foods_order_detail.get(i);
                txt += "<tr  class='style14'><td width='15' align='center'>"+foods_order_detail.getRoomId()+"</td>"
                    +"<td width='15' align='center'>"+foods_order_detail.getBedId()+"</td>"
                    +"<td width='60' align='center'><A HREF='javascript:changesubmitfoodsorderdetailshow("
        +foods_order_detail.getFoodsOrderDetailId()+")'  >"+foods_order_detail.getPatientHnNo()+"</A></td>"
                    +"<td width='160' align='left'>"+foods_order_detail.getPatientFullnamet()+"</td>"
                    +"<td width='160' align='left'>"+foods_order_detail.getTypeFoodsNamet()+"</td>"
                    +"<td width='160' align='left'>"+foods_order_detail.getRemark()+"</td></tr>";
            if(i==4){
                sql="";
            }
        }
